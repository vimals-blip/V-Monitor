...
export class ConfigurationService {
  ...
  constructor(@InjectRepository(ConfigurationVersionEntity) private repo: Repository<ConfigurationVersionEntity>) {}

  async findAll(query: PaginationDto, user: any) {
    const where: FindOptionsWhere<ConfigurationVersionEntity> = {};
    // Tenant isolation
    if (user.tenantId) (where as any).tenantId = user.tenantId;
    else (where as any).organizationId = user.organizationId;
    if (query.search) (where as any).resourceType = Like(`%${query.search}%`);
