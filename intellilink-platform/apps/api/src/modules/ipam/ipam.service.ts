...
export class IpamService {
  ...
  constructor(@InjectRepository(IpPoolEntity) private repo: Repository<IpPoolEntity>) {}

  async findAll(query: PaginationDto, user: any) {
    const where: FindOptionsWhere<IpPoolEntity> = {};
    // Tenant isolation
  ...
  async findOne(id: string, user: any) {
    ...
    if (!entity) throw new NotFoundException('Ipam not found');
