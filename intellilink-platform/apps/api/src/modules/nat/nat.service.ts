...
export class NatService {
  ...
  constructor(@InjectRepository(NatRuleEntity) private repo: Repository<NatRuleEntity>) {}

  async findAll(query: PaginationDto, user: any) {
    const where: FindOptionsWhere<NatRuleEntity> = {};
    // Tenant isolation
  ...
  async findOne(id: string, user: any) {
    ...
    if (!entity) throw new NotFoundException('Nat not found');
