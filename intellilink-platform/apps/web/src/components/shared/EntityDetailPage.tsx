    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 bg-[#121824] border border-[#222E45] rounded-xl p-6 space-y-4">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Entity Configuration & Attributes</h2>
            <div className="grid grid-cols-2 gap-4 text-xs">
              {Object.entries(data || {})
                .filter(([k]) => typeof data[k] !== 'object')
                .map(([k, v]) => (
                  <div key={k} className="p-3 bg-[#0D121D] rounded-lg border border-[#222E45]">
                    <span className="block text-[10px] uppercase font-semibold text-slate-500">{k}</span>
                    <span className="font-mono text-slate-200 mt-1 block truncate">{String(v)}</span>
                  </div>
                ))}
            </div>
          </div>
          ...
          <div className="bg-[#121824] border border-[#222E45] rounded-xl p-6 space-y-4">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Telemetry & Operations</h2>
            <div className="space-y-3">
              <div className="p-3 bg-[#0D121D] rounded-lg border border-[#222E45] text-xs">
                <span className="text-slate-400 block mb-1">State Machine</span>
                <span className="font-mono text-emerald-400 font-bold">SYNCHRONIZED</span>
              </div>
              <div className="p-3 bg-[#0D121D] rounded-lg border border-[#222E45] text-xs">
                <span className="text-slate-400 block mb-1">Adapter Health</span>
                <span className="font-mono text-cyan-400 font-bold">OPERATIONAL</span>
              </div>
            </div>
          </div>
        </div>
