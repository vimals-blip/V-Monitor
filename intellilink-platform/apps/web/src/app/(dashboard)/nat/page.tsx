...
export default function NATRulesPage() {
  ...
  const { data: natResponse, isLoading, refetch } = useQuery({
    queryKey: ['nat-list', search, page, pageSize],
    queryFn: async () => {
      const res = await apiClient.get(`/nat?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
      return res.data;
    },
    refetchInterval: 5000,
  });

  const data = Array.isArray(natResponse?.data)
    ? natResponse.data
    : Array.isArray(natResponse)
    ? natResponse
    : [];
  const totalNat = natResponse?.total ?? data.length;
  const totalPages = natResponse?.totalPages ?? Math.max(1, Math.ceil(totalNat / pageSize));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Carrier-Grade NAT & Port Forwarding</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Total NAT Rules</div>
          <div className="text-lg font-bold text-slate-900 dark:text-white mt-1">{totalNat}</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active & Deployed</div>
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">
            {(data || []).filter((n: any) => n.status === 'DEPLOYED').length} Active
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Public IP Pools</div>
          <div className="text-lg font-bold text-cyan-600 dark:text-cyan-400 mt-1">103.14.22.0/24</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Concurrent NAT Sessions</div>
      ...
      <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">
          State: <span className="text-emerald-600 dark:text-emerald-400 font-bold">CONNTRACK_OFFLOAD_NOMINAL</span>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:bg-[#161F30] transition-colors">
                  ...
                  <td className="p-3.5 font-mono text-slate-600 dark:text-slate-300">{item.sourceAddress || '—'}</td>
                  <td className="p-3.5 font-mono text-slate-500 dark:text-slate-400">{item.destinationAddress || '0.0.0.0/0'}</td>
                  <td className="p-3.5 font-mono text-emerald-600 dark:text-emerald-400 font-semibold">{item.translatedAddress || '—'}</td>
                  ...
                  <td className="p-3.5 text-center">
                    {item.status !== 'DEPLOYED' ? (
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-[#1A2333] hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 text-[11px] font-medium transition-colors inline-flex items-center gap-1"
                      >
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-white dark:bg-[#121824] px-5 py-4 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Create Address Translation (NAT) Rule</h2>
              </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">NAT Type *</label>
                  <select
                    value={formData.type}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Source Address / CIDR *</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Destination Address / CIDR *</label>
                  <input
                    required
                    type="text"
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Translated Public Target IP *</label>
                <input
                  required
                  type="text"
                  ...
                  className="w-full bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg px-3 py-2 text-emerald-600 dark:text-emerald-400 font-mono focus:outline-none focus:border-cyan-500"
                />
              </div>
