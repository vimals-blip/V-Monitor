...
export default function AiAssistantPage() {
  ...
  return (
    <div className="h-[calc(100vh-7rem)] flex gap-3">
      {/* Sessions Sidebar */}
      <div className="w-64 bg-[#0B0F17] border border-[#222E45] rounded-xl flex flex-col overflow-hidden hidden lg:flex">
        <div className="p-3 border-b border-[#222E45] flex items-center justify-between bg-[#121824]">
          <div className="flex items-center gap-2">
            <Bot className="w-4 h-4 text-cyan-400" />
            <span className="text-xs font-bold text-white tracking-wide">AIOps Sessions</span>
          </div>
          <button
            onClick={handleNewChat}
            className="p-1.5 rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 border border-cyan-500/30 text-[11px] flex items-center gap-1 font-medium transition-all"
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 text-xs">
          {sessions.length === 0 ? (
            <div className="text-slate-500 text-center py-8 text-[11px]">No previous sessions</div>
          ) : (
            sessions.map((s) => (
              <button
                key={s.id}
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg border transition-all truncate block ${
                  currentSessionId === s.id
                    ? 'bg-cyan-950/40 border-cyan-500/50 text-cyan-300 font-medium'
                    : 'bg-[#121824]/60 border-[#222E45] text-slate-300 hover:bg-[#1A2333] hover:text-white'
                }`}
              >
        ...
        <div className="p-3 border-t border-[#222E45] bg-[#121824] text-[10px] text-slate-400 space-y-1">
      ...
      {/* Main Chat Interface */}
      <div className="flex-1 flex flex-col bg-[#121824] border border-[#222E45] rounded-xl overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="p-3.5 border-b border-[#222E45] flex items-center justify-between bg-[#0D121D]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center text-black shadow-lg shadow-cyan-500/20">
              <Bot className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-bold text-white tracking-tight">Intellilink AIOps NOC Assistant</h2>
          ...
          <div className="flex items-center gap-2 text-xs text-emerald-400 bg-emerald-950/40 px-3 py-1 rounded-full border border-emerald-800">
        ...
        {/* Quick Action Chips */}
        <div className="px-4 py-2 border-b border-[#1A2333] bg-[#0A0E17] flex items-center gap-2 overflow-x-auto text-[11px]">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-[#121824] border border-[#222E45] text-slate-300 hover:text-cyan-300 hover:border-cyan-500/40 transition-all shrink-0 flex items-center gap-1"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-[#121824] border border-[#222E45] text-slate-300 hover:text-amber-300 hover:border-amber-500/40 transition-all shrink-0 flex items-center gap-1"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-[#121824] border border-[#222E45] text-slate-300 hover:text-purple-300 hover:border-purple-500/40 transition-all shrink-0 flex items-center gap-1"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-[#121824] border border-[#222E45] text-slate-300 hover:text-emerald-300 hover:border-emerald-500/40 transition-all shrink-0 flex items-center gap-1"
          >
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-[#121824] border border-[#222E45] text-slate-300 hover:text-blue-300 hover:border-blue-500/40 transition-all shrink-0 flex items-center gap-1"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 font-sans">
          {messages.map((m, idx) => (
            <div key={idx} className={`flex gap-3 max-w-3xl ${m.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
              ...
              <div
                className={`rounded-xl p-4 text-xs leading-relaxed space-y-3 shadow-md ${
                  m.role === 'user'
                    ? 'bg-cyan-600 text-white'
                    : 'bg-[#0D121D] border border-[#222E45] text-slate-200'
                }`}
              >
                ...
                {m.toolCalls && m.toolCalls.length > 0 && (
                  <div className="space-y-1.5">
                    {m.toolCalls.map((tc, tcIdx) => (
                      <div
                        key={tcIdx}
                        className="bg-[#121824] border border-[#222E45] px-2.5 py-1.5 rounded-lg flex items-center justify-between text-[11px] font-mono text-slate-300"
                      >
                        <div className="flex items-center gap-1.5">
                          <Terminal className="w-3.5 h-3.5 text-cyan-400" />
                          <span className="text-cyan-400 font-semibold">{tc.tool}</span>
                        </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-[#222E45] space-y-1.5">
                    ...
                    <div className="space-y-1">
                      {m.evidence.map((ev, eIdx) => (
                        <div
                          key={eIdx}
                          className={`flex items-start gap-2 text-[11px] font-mono p-2 rounded border ${
                            ev.type === 'FACT'
                  ...
                  <div className="mt-3 pt-2.5 border-t border-[#222E45] flex flex-wrap gap-1.5">
                    {m.suggestedActions.map((act, aIdx) => (
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-lg bg-[#162338] hover:bg-[#1E304D] border border-cyan-500/40 text-cyan-300 text-[11px] font-medium flex items-center gap-1.5 transition-all shadow-sm"
                      >
                        <span>{act.label}</span>
          ...
          {loading && (
            <div className="flex items-center gap-2.5 text-xs text-slate-400 animate-pulse bg-[#0D121D] p-3 rounded-lg border border-[#222E45] max-w-sm">
        ...
        {/* Input Bar */}
        <form onSubmit={sendMessage} className="p-3 border-t border-[#222E45] bg-[#0D121D] flex gap-2">
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Ask AIOps: 'Why is gateway degraded?', 'Ping 8.8.8.8', 'Show critical alerts', 'Detect telemetry anomalies'..."
            className="flex-1 bg-[#121824] border border-[#222E45] rounded-lg px-4 py-2.5 text-xs text-white focus:outline-none focus:border-cyan-500 transition-colors font-sans"
          />
          <button
            type="submit"
            disabled={loading}
            className="px-4 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-black font-semibold text-xs flex items-center gap-1.5 transition-colors disabled:opacity-50 shadow-lg shadow-cyan-500/20"
          >
            <span>Query</span>
