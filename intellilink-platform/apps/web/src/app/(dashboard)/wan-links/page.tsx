...
export default function WANLinksPage() {
  ...
  const [testModal, setTestModal] = useState<{ open: boolean; link: any; result?: any; loading?: boolean } | null>(null);
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['wan-links-list', search],
    queryFn: async () => {
      const res = await apiClient.get('/wan-links?search=' + search);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight">WAN Circuits & Hybrid Backhauls</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Total Circuits</div>
          <div className="text-lg font-bold text-white mt-1">{(data || []).length}</div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Starlink LEO Satellite</div>
          <div className="text-lg font-bold text-cyan-400 mt-1">
            {(data || []).filter((w: any) => w.type === 'SATELLITE').length} Active
          </div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Dedicated Optical Fiber</div>
          <div className="text-lg font-bold text-emerald-400 mt-1">
            {(data || []).filter((w: any) => w.type === 'FIBER').length} Active
          </div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Avg Circuit Packet Loss</div>
          <div className="text-lg font-bold text-emerald-400 mt-1">0.12%</div>
        </div>
      </div>
      ...
      <div className="bg-[#121824] border border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-400 font-mono">
          SD-WAN Engine: <span className="text-emerald-400 font-bold">SUB_SECOND_FAILOVER</span>
        </div>
      </div>
                  ...
                  <td className="p-3.5">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                      item.type === 'SATELLITE'
                        ...
                        : 'bg-[#1C263A] text-slate-300'
                    }`}>
                      {item.type || 'FIBER'}
                    </span>
                  </td>
                  <td className="p-3.5 text-slate-300">{item.providerName || 'Carrier'}</td>
      ...
      {testModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0B0F17] border border-[#222E45] rounded-xl max-w-2xl w-full overflow-hidden shadow-2xl">
            <div className="bg-[#121824] px-4 py-3 border-b border-[#222E45] flex items-center justify-between">
              ...
              <button onClick={() => setTestModal(null)} className="text-slate-400 hover:text-white p-1">
                <X className="w-4 h-4" />
              </button>
            </div>

            {/* Target Probe Selector Tabs */}
            <div className="bg-[#0e1420] px-4 py-2 border-b border-[#222E45] flex items-center gap-2 text-xs">
              <span className="text-slate-400 font-medium text-[11px] mr-1">Probe Target:</span>
              <button
                ...
                className={`px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
                  testModal.targetMode === 'GATEWAY_CPE'
                    ...
                    : 'bg-[#162032] text-slate-300 hover:text-white border border-[#222E45]'
                }`}
              >
                Physical Edge CPE
              </button>
              ...
              {testModal.loading ? (
                ...
                <div className="space-y-4">
                  <div className="p-3 rounded-lg bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 text-xs flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-mono text-emerald-400/80">{testModal.result?.targetDescription}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      ...
                      <span className="text-emerald-400 font-bold text-base">{testModal.result?.latencyMs} ms</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">JITTER VARIANCE</span>
                      <span className="text-cyan-400 font-bold text-base">{testModal.result?.jitterMs} ms</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">DOWNLOAD BANDWIDTH</span>
                      <span className="text-purple-400 font-bold text-base">{testModal.result?.provisionedBandwidthDownMbps} Mbps</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">UPLOAD BANDWIDTH</span>
                  ...
                  <div className="space-y-1">
                    <span className="text-[11px] text-slate-400 font-medium">Real Linux Kernel Probe Trace:</span>
                    <pre className="p-3 bg-[#05080E] rounded-lg border border-[#222E45] text-emerald-400 text-[11px] overflow-x-auto leading-relaxed">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">PACKET LOSS RATE</span>
                    ...
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">LIVE THROUGHPUT</span>
                      <span className="text-slate-500 font-bold text-base">0.00 Mbps</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">BFD OVERLAY STATE</span>
                  ...
                  <div className="space-y-1">
                    <span className="text-[11px] text-slate-400 font-medium">Real Host Kernel Diagnostic Trace:</span>
                  ...
                  <div className="p-3 rounded-lg bg-[#121824] border border-[#222E45] text-slate-400 text-[11px] space-y-1">
                    <span className="text-white font-medium block">ℹ️ Carrier Operations Note:</span>
                    <p>
            ...
            <div className="bg-[#121824] px-4 py-3 border-t border-[#222E45] flex items-center justify-between">
              <button
                ...
                disabled={testModal.loading}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#1A2333] hover:bg-[#222E45] text-cyan-400 text-xs font-medium border border-cyan-500/20"
              >
          ...
          <div className="bg-[#0B0F17] border border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-[#121824] px-5 py-4 border-b border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-white">Enroll New WAN Circuit</h2>
              </div>
            ...
            >
              <div>
                <label className="text-slate-300 font-medium block mb-1">Circuit Name *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Medium / Type *</label>
                  <select
                    value={formData.type}
                ...
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Provider Name *</label>
                  <input
                    required
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Download Bandwidth (Mbps)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Upload Bandwidth (Mbps)</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="text-slate-300 font-medium block mb-1">Assign to Edge Gateway *</label>
                <select
