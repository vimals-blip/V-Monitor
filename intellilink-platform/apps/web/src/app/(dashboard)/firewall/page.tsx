...
export default function FirewallRulesPage() {
  ...
  const { data: firewallResponse, isLoading, refetch } = useQuery({
    queryKey: ['firewall-list', search, page, pageSize],
    queryFn: async () => {
      const res = await apiClient.get(`/firewall?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
      return res.data;
    },
    refetchInterval: 5000,
  });

  const data = Array.isArray(firewallResponse?.data)
    ? firewallResponse.data
    : Array.isArray(firewallResponse)
    ? firewallResponse
    : [];
  const totalRules = firewallResponse?.total ?? data.length;
  const totalPages = firewallResponse?.totalPages ?? Math.max(1, Math.ceil(totalRules / pageSize));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Zero-Trust Edge Firewall & ACLs</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Total Rules</div>
          <div className="text-lg font-bold text-slate-900 dark:text-white mt-1">{totalRules}</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active In NFTables</div>
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">
            {(data || []).filter((r: any) => r.enabled).length} Enforced
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Zero-Trust Drops</div>
          <div className="text-lg font-bold text-red-600 dark:text-red-400 mt-1">
            {(data || []).filter((r: any) => r.action === 'DENY').length} Block Rules
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Packet Inspection Engine</div>
          <div className="text-lg font-bold text-cyan-600 dark:text-cyan-400 mt-1">eBPF / XDP Nominal</div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">
          Inspection Mode: <span className="text-cyan-600 dark:text-cyan-400 font-bold">STATEFUL_CONNECTION_TRACKING</span>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:bg-[#161F30] transition-colors">
                  ...
                  <td className="p-3.5 font-mono text-cyan-600 dark:text-cyan-400">{item.source || '0.0.0.0/0'}</td>
                  <td className="p-3.5 font-mono text-slate-600 dark:text-slate-300">{item.destination || '0.0.0.0/0'}</td>
                  ...
                  <td className="p-3.5 font-mono text-slate-500 dark:text-slate-400">#{item.priority || 100}</td>
                  <td className="p-3.5 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <button
                        ...
                        className="px-2 py-1 rounded bg-[#1A2333] hover:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30 text-[11px] font-medium transition-colors"
                      >
                        Test Match
                      </button>
                    </div>
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#1E293B] rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl space-y-0">
            ...
            <div className="bg-white dark:bg-[#0F172A] px-6 py-4 border-b border-slate-200 dark:border-[#1E293B] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">{matchModal.rule?.name}</h2>
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded font-medium ${
                      matchModal.rule?.action === 'ALLOW'
                        ? 'bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-800/60'
            ...
            <div className="p-6 space-y-4 font-mono text-xs">
              {matchModal.loading ? (
                <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 font-semibold font-sans">Traversing Linux netfilter rule chains...</p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-2">
                    <span className="text-slate-500 dark:text-slate-400 text-[10px] uppercase font-sans tracking-wider block">
                    ...
                    <div className="flex items-center justify-between bg-slate-50 dark:bg-[#090D16] p-3 rounded-lg border border-slate-200 dark:border-[#1E293B] text-[11px]">
                      ...
                      <div className="flex flex-col items-center px-2">
                        <span className="text-[9px] text-blue-600 dark:text-blue-400 font-mono">{matchModal.rule?.protocol?.toUpperCase()}</span>
                        <ArrowRight className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                      </div>
                      <div className="text-center">
                        <span className="text-slate-500 text-[10px] block">DESTINATION CIDR</span>
                        <span className="text-emerald-600 dark:text-emerald-400 font-bold">{matchModal.rule?.destination || '0.0.0.0/0'}</span>
                      </div>
                      <div className="flex flex-col items-center px-2">
                      ...
                      <div className="text-center">
                        ...
                        <span className="text-slate-900 dark:text-white font-bold">{matchModal.rule?.ports || 'any'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div className="bg-white dark:bg-[#0F172A] p-3 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      ...
                      <span className={`font-bold mt-0.5 block ${matchModal.rule?.action === 'ALLOW' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-400'}`}>
                    ...
                    <div className="bg-white dark:bg-[#0F172A] p-3 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 font-bold mt-0.5 block">FORWARD</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-sans">
                      ...
                      <span className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">SYNTHESIZED</span>
                    </div>
                    <pre className="p-3.5 bg-[#05080E] rounded-xl border border-slate-200 dark:border-[#1E293B] text-slate-600 dark:text-slate-300 text-[11px] overflow-x-auto leading-relaxed">
            ...
            <div className="bg-white dark:bg-[#0F172A] px-6 py-3.5 border-t border-slate-200 dark:border-[#1E293B] flex items-center justify-between font-sans">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setMatchModal(null)}
                  className="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white text-xs font-medium transition-colors shadow-sm"
                >
                  Close
                </button>
              </div>
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-white dark:bg-[#121824] px-5 py-4 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Create Firewall ACL Rule</h2>
              </div>
            ...
            >
              <div>
                <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Rule Name *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Source CIDR *</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Destination CIDR *</label>
                  <input
                    required
                    type="text"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Protocol *</label>
                  <select
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Ports</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Action *</label>
                  <select
                    value={formData.action}
                    onChange={(e) => setFormData({ ...formData, action: e.target.value })}
                    className="w-full bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg px-3 py-2 text-slate-900 dark:text-white font-bold focus:outline-none focus:border-cyan-500"
                  >
