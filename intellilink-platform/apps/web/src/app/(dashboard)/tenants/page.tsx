...
export default function TenantsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get('/tenants?search=' + search);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight">Enterprise & Government Tenants</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Total Tenants</div>
          <div className="text-lg font-bold text-white mt-1">{(data || []).length}</div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Active Production</div>
          <div className="text-lg font-bold text-emerald-400 mt-1">
            {(data || []).filter((t: any) => t.status === 'ACTIVE').length}
          </div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Banking & Gov Segments</div>
          <div className="text-lg font-bold text-cyan-400 mt-1">
            {(data || []).filter((t: any) => t.type === 'BANKING' || t.type === 'GOVERNMENT').length}
          </div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Customer SLA Status</div>
          <div className="text-lg font-bold text-emerald-400 mt-1">99.98% SLA</div>
        </div>
      </div>
      ...
      <div className="bg-[#121824] border border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-400 font-mono">
          Tenant Isolation: <span className="text-cyan-400 font-bold">STRICT_MLS</span>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="hover:bg-[#161F30] transition-colors">
                  ...
                  <td className="p-3.5 font-mono text-cyan-400">{item.slug || '—'}</td>
                  <td className="p-3.5">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#1C263A] text-slate-200 border border-[#222E45]">
          ...
          <div className="bg-[#0B0F17] border border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-[#121824] px-5 py-4 border-b border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-white">Enroll New Enterprise Tenant</h2>
              </div>
            ...
            >
              <div>
                <label className="text-slate-300 font-medium block mb-1">Tenant Organization Name *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Tenant Slug *</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Industry Sector *</label>
                  <select
                    value={formData.type}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Max Branch Sites</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-slate-300 font-medium block mb-1">Max Edge Gateways</label>
                  <input
                    type="number"
