...
export default function PoliciesPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['policies-list', search],
    queryFn: async () => {
      const res = await apiClient.get('/policies?search=' + search);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-white tracking-tight">SD-WAN Governance & Traffic Steering Policies</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Total Policies</div>
          <div className="text-lg font-bold text-white mt-1">{(data || []).length}</div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Enforced on Gateways</div>
          <div className="text-lg font-bold text-emerald-400 mt-1">
            {(data || []).filter((p: any) => p.isActive).length} Enforced
          </div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Automated Steering</div>
          <div className="text-lg font-bold text-cyan-400 mt-1">Fiber ↔ Starlink</div>
        </div>
        <div className="bg-[#121824] border border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-400 uppercase font-semibold">Fleet Propagation</div>
      ...
      <div className="bg-[#121824] border border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-400 font-mono">
          Engine: <span className="text-emerald-400 font-bold">AUTONOMOUS_POLICY_CONTROLLER</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0B0F17] border border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl space-y-4">
            <div className="bg-[#121824] px-4 py-3 border-b border-[#222E45] flex items-center justify-between">
              ...
              <button onClick={() => setEnforceModal(null)} className="text-slate-400 hover:text-white p-1">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-3 font-mono text-xs">
              {enforceModal.loading ? (
                <div className="py-10 flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-slate-300 font-semibold">Broadcasting policy bytecode to edge gateways...</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">TARGET GATEWAYS</span>
                      <span className="text-white font-bold">{enforceModal.result?.gatewaysTargeted || 0} Edge Routers</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">SYNC CONFIRMATION</span>
                      <span className="text-emerald-400 font-bold">{enforceModal.result?.gatewaysSynchronized || 0} / {enforceModal.result?.gatewaysTargeted || 0} Confirmed</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">BROADCAST LATENCY</span>
                      <span className="text-cyan-400 font-bold">{enforceModal.result?.propagationLatency}</span>
                    </div>
                    <div className="p-3 bg-[#121824] rounded border border-[#222E45]">
                      <span className="text-slate-400 block text-[10px]">ENFORCEMENT ENGINE</span>
                      <span className="text-purple-400 font-bold">{enforceModal.result?.enforcementEngine || 'Linux TC & Netfilter'}</span>
                    </div>
                  </div>
                  <div className="p-3 bg-[#05080E] rounded border border-[#222E45] text-slate-400 text-[11px] space-y-1">
                    <div>✓ Policy revision: <span className="text-white font-mono">v{enforceModal.result?.version}</span></div>
                    <div>✓ Signature: <span className="text-cyan-400 font-mono">{enforceModal.result?.sha256}</span></div>
                    <div>✓ Edge reboot: <span className="text-white">Zero edge reboot required (dynamic in-kernel rule replacement)</span></div>
                    <div>✓ Live state: <span className="text-emerald-400 font-semibold">{enforceModal.result?.status}</span></div>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-[#0B0F17] border border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-[#121824] px-5 py-4 border-b border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-white">Create SD-WAN Policy</h2>
              </div>
            ...
            >
              ...
              <div>
                <label className="text-slate-300 font-medium block mb-1">Policy Category *</label>
                <select
                  value={formData.type}
              ...
              <div>
                <label className="text-slate-300 font-medium block mb-1">Enforcement Intent Description *</label>
                <textarea
                  required
                  rows={3}
