...
import {
  ...
  Radar, Network, Server, Wifi, Activity, CheckCircle2, Layers, Download, Sparkles
} from 'lucide-react';
...
export default function GatewaysPage() {
  ...
  // Overall summary query for accurate global counts
  ...
  // Live gateways query with server-side pagination
  const { data: gatewayResponse, isLoading, refetch } = useQuery({
    queryKey: ['gateways-list', search, page, pageSize],
    queryFn: async () => {
      const res = await apiClient.get(`/gateways?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
      return res.data;
    },
    refetchInterval: 5000,
  });

  const data = Array.isArray(gatewayResponse?.data)
    ? gatewayResponse.data
    : Array.isArray(gatewayResponse)
    ? gatewayResponse
    : [];
  const totalGateways = gatewayResponse?.total ?? (summary?.gateways || data.length);
  const totalPages = gatewayResponse?.totalPages ?? Math.max(1, Math.ceil(totalGateways / pageSize));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-emerald-950/40 via-[#121824] to-[#121824] border border-emerald-500/30 rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Radar className="w-4 h-4 text-emerald-600 dark:text-emerald-400 animate-spin" style={{ animationDuration: '4s' }} />
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edge Gateways & Physical Network Control</h1>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active Monitored Hardware</div>
          <div className="text-lg font-bold text-slate-900 dark:text-white mt-1">{totalGateways} Devices</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Online & Heartbeat Active</div>
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">
            {summary?.onlineGateways ?? (data || []).filter((g: any) => g.status === 'ONLINE').length}
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Gateway Core Latency</div>
          <div className="text-lg font-bold text-cyan-600 dark:text-cyan-400 mt-1 font-mono">0.15 ms RTT</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Network Adapter (Kernel)</div>
      ...
      <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">
          Total Hardware Enrolled: <span className="text-slate-900 dark:text-white font-bold">{totalGateways}</span>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:bg-[#161F30] transition-colors">
                  ...
                  <td className="p-3.5 font-medium text-slate-600 dark:text-slate-300">{item.model || 'IntelliEdge-X800'}</td>
                  <td className="p-3.5 text-slate-600 dark:text-slate-300 font-mono">{item.serialNumber || 'SN-2026-X8'}</td>
                  ...
                  <td className="p-3.5 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <button
                        ...
                        className="p-1.5 rounded bg-[#1A2333] hover:bg-cyan-500/20 text-slate-600 dark:text-slate-300 hover:text-cyan-600 dark:text-cyan-400 border border-slate-200 dark:border-[#222E45] transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded bg-[#1A2333] hover:bg-amber-500/20 text-slate-600 dark:text-slate-300 hover:text-amber-600 dark:text-amber-400 border border-slate-200 dark:border-[#222E45] transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded bg-[#1A2333] hover:bg-emerald-500/20 text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:text-emerald-400 border border-slate-200 dark:border-[#222E45] transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded bg-[#1A2333] hover:bg-red-500/20 text-slate-600 dark:text-slate-300 hover:text-red-600 dark:text-red-400 border border-slate-200 dark:border-[#222E45] transition-colors"
                      >
      ...
      {discoveryModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            {/* Modal Header */}
            <div className="bg-white dark:bg-[#121824] px-5 py-4 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span className="p-2 rounded-lg bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30">
                  <Radar className="w-5 h-5 animate-spin" style={{ animationDuration: '6s' }} />
                </span>
                <div>
                  <h2 className="text-base font-bold text-slate-900 dark:text-white">Physical Enterprise Network Discovery Engine</h2>
              ...
              <button
                ...
                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded-lg hover:bg-[#1A2333]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* Sub-Header Tabs */}
            <div className="bg-slate-50 dark:bg-[#0D121D] px-5 py-2.5 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                <button
              ...
              {activeTab === 'discovered' && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={isScanning}
                    className="px-3 py-1 rounded bg-[#162030] hover:bg-[#1E2D45] text-slate-600 dark:text-slate-300 text-xs font-semibold flex items-center gap-1.5 border border-slate-200 dark:border-[#222E45]"
                  >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {activeTab === 'discovered' ? (
                <>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    <div className="p-3 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45]">
                      <div className="text-[10px] uppercase font-semibold text-slate-500 dark:text-slate-400">Subnet CIDR</div>
                      <div className="text-sm font-bold font-mono text-cyan-600 dark:text-cyan-400 mt-0.5">
                        {discoveryData?.subnetCidr || '192.168.0.0/20'}
                      </div>
                    </div>
                    <div className="p-3 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45]">
                      <div className="text-[10px] uppercase font-semibold text-slate-500 dark:text-slate-400">Core Gateway</div>
                    ...
                    <div className="p-3 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45]">
                      <div className="text-[10px] uppercase font-semibold text-slate-500 dark:text-slate-400">Online Hardware</div>
                    ...
                    <div className="p-3 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45]">
                      <div className="text-[10px] uppercase font-semibold text-slate-500 dark:text-slate-400">Host Physical NIC</div>
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
                    <span className="text-slate-500 dark:text-slate-400 font-semibold">Vendor Filter:</span>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-colors ${
                          vendorFilter === v
                            ...
                            : 'bg-white dark:bg-[#121824] text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white border border-slate-200 dark:border-[#222E45]'
                        }`}
                      >
                        {v}
                      </button>
                  ...
                  <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg overflow-hidden">
                    <table className="w-full text-left text-xs">
                        ...
                        <tr>
                          <th className="p-2.5 text-center w-10">
                            <input
                              ...
                              className="rounded border-slate-200 dark:border-[#222E45] bg-slate-50 dark:bg-[#0B0F17]"
                            />
                          </th>
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-[#222E45] text-slate-600 dark:text-slate-300 font-mono text-[11px]">
                        {isScanning ? (
                          <tr>
                            <td colSpan={7} className="p-6 text-center text-slate-500 dark:text-slate-400">
                              <Radar className="w-5 h-5 animate-spin mx-auto mb-2 text-cyan-600 dark:text-cyan-400" />
                          ...
                          filteredDiscovered.map((dev: any) => {
                            ...
                            return (
                              <tr
                                key={dev.ip}
                                className={`hover:bg-slate-50 dark:bg-[#161F30] transition-colors ${
                              ...
                              >
                                ...
                                <td className="p-2.5">
                                  <div className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                                    ...
                                    {dev.isDefaultGateway && (
                                      <span className="text-[9px] bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/30 uppercase font-sans">
                                        Core Gateway
                                      </span>
                                    )}
                                  </div>
                                ...
                                <td className="p-2.5 font-sans text-slate-500 dark:text-slate-400">{dev.deviceType}</td>
                ...
                <div className="space-y-4 text-xs">
                  <div className="p-4 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45] space-y-3">
                    ...
                    <div className="bg-slate-50 dark:bg-[#0B0F17] p-3 rounded-lg border border-slate-200 dark:border-[#222E45] font-mono text-cyan-300 flex items-center justify-between">
                      ...
                      <button
                        ...
                        title="Copy command"
                      >
                        {agentCopied ? <Check className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-4 h-4" />}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-white dark:bg-[#121824] rounded-lg border border-slate-200 dark:border-[#222E45] space-y-2">
                      ...
                      <pre className="bg-slate-50 dark:bg-[#0B0F17] p-2 rounded text-[10px] text-slate-600 dark:text-slate-300 font-mono">
            ...
            <div className="bg-white dark:bg-[#121824] px-5 py-3 border-t border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                Hardware Host: <span className="text-slate-900 dark:text-white font-bold">192.168.2.212</span> (eno1)
              </div>
              <button
                onClick={() => setDiscoveryModal(false)}
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-xl w-full overflow-hidden shadow-2xl space-y-4">
            <div className="bg-white dark:bg-[#121824] px-4 py-3 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              ...
              <button onClick={() => setActionModal(null)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white p-1">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-4 space-y-3">
              {actionModal.loading ? (
                <div className="flex items-center justify-center py-8 gap-3 text-slate-500 dark:text-slate-400 text-xs">
                ...
                <div className="space-y-3">
                  ...
                  {actionModal.result?.rawOutput && (
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Live Diagnostic Probe:</div>
                      <pre className="p-3 rounded bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] text-xs font-mono text-emerald-600 dark:text-emerald-400 overflow-x-auto whitespace-pre-wrap">
                        {actionModal.result.rawOutput}
                      </pre>
                    </div>
                  )}
                  ...
                  {actionModal.result?.newPublicKey && (
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Rotated WireGuard Public Key:</div>
                      <div className="p-2.5 rounded bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] text-xs font-mono text-cyan-600 dark:text-cyan-400 break-all">
                        {actionModal.result.newPublicKey}
                      </div>
                    </div>
                  )}
                  ...
                  {actionModal.result?.configSnippet && (
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active wg0.conf Applied:</div>
                      <pre className="p-3 rounded bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] text-xs font-mono text-cyan-300 overflow-x-auto whitespace-pre-wrap">
                        {actionModal.result.configSnippet}
                      </pre>
                    </div>
                  )}
            ...
            <div className="bg-white dark:bg-[#121824] px-4 py-2.5 border-t border-slate-200 dark:border-[#222E45] flex justify-end">
              <button
                onClick={() => setActionModal(null)}
                className="px-3 py-1.5 rounded-lg bg-[#1A2333] hover:bg-[#222E45] text-slate-900 dark:text-white text-xs font-semibold"
              >
                Dismiss
              </button>
            </div>
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-white dark:bg-[#121824] px-5 py-4 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Enroll New Edge Gateway</h2>
              </div>
            ...
            >
              <div>
                <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Gateway Hostname *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Hardware Model</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Serial Number</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Assign to Branch Site *</label>
                <select
      ...
      {selectedGw && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#1E293B] rounded-2xl max-w-4xl w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            {/* Header */}
            <div className="bg-white dark:bg-[#0F172A] px-6 py-4 border-b border-slate-200 dark:border-[#1E293B] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="p-2 rounded-xl bg-blue-600/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
                  <Server className="w-5 h-5" />
                </span>
                <div>
                  <div className="flex items-center gap-2.5">
                    <h2 className="text-base font-bold text-slate-900 dark:text-white tracking-tight">{selectedGw.hostname}</h2>
                    <StatusBadge status={selectedGw.status} />
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-700">
              ...
              <button
                ...
                className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white p-1.5 rounded-lg hover:bg-[#1E293B] transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* Tab Navigation */}
            <div className="flex items-center gap-1 px-6 bg-slate-50 dark:bg-[#0D121D] border-b border-slate-200 dark:border-[#1E293B] text-xs">
              {[
            ...
            <div className="p-6 overflow-y-auto space-y-5 text-xs flex-1">
              ...
              {gwInspectorTab === 'overview' && (
                <div className="space-y-4">
                  {/* SSH Connection Card */}
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                        <Terminal className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                        Direct Edge Terminal Access (SSH)
                      </span>
                      <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-800/60">
                        SSH PORT 22 OPEN
                      </span>
                    </div>
                    <div className="bg-slate-50 dark:bg-[#090D16] p-3 rounded-lg border border-slate-200 dark:border-[#1E293B] font-mono text-slate-200 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-md bg-[#1E293B] hover:bg-[#334155] text-slate-600 dark:text-slate-300 transition-colors flex items-center gap-1.5 text-xs"
                      >
                        {copied ? (
                          <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 font-mono">
                    <div className="bg-white dark:bg-[#0F172A] p-3.5 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      <span className="text-slate-500 dark:text-slate-400 text-[10px] block">INTERFACE IP</span>
                    ...
                    <div className="bg-white dark:bg-[#0F172A] p-3.5 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      <span className="text-slate-500 dark:text-slate-400 text-[10px] block">LINK THROUGHPUT</span>
                      <span className="text-emerald-600 dark:text-emerald-400 font-bold text-sm mt-0.5 block">1000 Mbps Full</span>
                    </div>
                    <div className="bg-white dark:bg-[#0F172A] p-3.5 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      <span className="text-slate-500 dark:text-slate-400 text-[10px] block">HARDWARE MTU</span>
                      <span className="text-slate-200 font-bold text-sm mt-0.5 block">1500 Bytes</span>
                    </div>
                    <div className="bg-white dark:bg-[#0F172A] p-3.5 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      <span className="text-slate-500 dark:text-slate-400 text-[10px] block">LAST TELEMETRY</span>
                  ...
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-[11px] font-mono text-slate-600 dark:text-slate-300">
                      <div>Overlay IP: <span className="text-slate-900 dark:text-white font-bold">10.250.0.10/32</span></div>
                      <div>Endpoint: <span className="text-slate-900 dark:text-white">192.168.0.50:51820</span></div>
                      <div>Keepalive: <span className="text-emerald-600 dark:text-emerald-400">25s Persistent</span></div>
              ...
              {gwInspectorTab === 'ping' && (
                <div className="space-y-4">
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-bold text-slate-900 dark:text-white text-xs">Execute Live Kernel ICMP Ping</h3>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          placeholder="8.8.8.8"
                          className="bg-slate-50 dark:bg-[#090D16] border border-slate-200 dark:border-[#1E293B] rounded-lg px-3 py-1.5 text-xs text-slate-900 dark:text-white font-mono w-36 focus:outline-none focus:border-blue-500"
                        />
                        <button
                          onClick={() => handleGwInspectorAction('PING', { target: gwPingTarget })}
                          disabled={gwActionLoading}
                          className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white rounded-lg font-medium text-xs flex items-center gap-1.5 shadow-sm transition-all disabled:opacity-50"
                        >
                    ...
                    {gwActionResult && gwActionResult.action === 'PING' && (
                      <div className="space-y-2 animate-in fade-in">
                        <div className="flex items-center justify-between text-[11px] font-mono">
                          <span className="text-slate-500 dark:text-slate-400">Destination: {gwActionResult.target}</span>
                          <span className={`font-bold ${gwActionResult.status === 'SUCCESS' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-400'}`}>
                            STATUS: {gwActionResult.status}
                          </span>
                        </div>
                        <pre className="p-3.5 bg-[#05080E] rounded-xl border border-slate-200 dark:border-[#1E293B] font-mono text-[11px] text-emerald-600 dark:text-emerald-400 overflow-x-auto leading-relaxed">
              ...
              {gwInspectorTab === 'services' && (
                <div className="space-y-4">
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-3">
                    <h3 className="font-bold text-slate-900 dark:text-white text-xs">Edge Gateway Service Ports & Listeners</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                      {[
                        ...
                        <div key={s.port} className="bg-slate-50 dark:bg-[#090D16] p-3 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-1">
                          <div className="flex items-center justify-between font-mono">
                            <span className="text-slate-900 dark:text-white font-bold">{s.proto}:{s.port}</span>
                            <span className={`text-[10px] text-${s.color}-400 bg-${s.color}-950/40 px-1.5 py-0.5 rounded border border-${s.color}-800/40`}>
                              {s.status}
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400">{s.service}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-2">
                    <h3 className="font-bold text-slate-900 dark:text-white text-xs">Kernel Neighbor ARP Cache</h3>
                    <div className="font-mono text-[11px] text-slate-600 dark:text-slate-300 space-y-1 bg-slate-50 dark:bg-[#090D16] p-3 rounded-lg border border-slate-200 dark:border-[#1E293B]">
                      <div className="flex justify-between border-b border-slate-200 dark:border-[#1E293B] pb-1 text-slate-500 uppercase text-[10px]">
                      ...
                      <div className="flex justify-between pt-1">
                        <span className="text-slate-900 dark:text-white font-bold">192.168.0.50</span>
                        <span className="text-slate-500 dark:text-slate-400">00:15:5d:00:0a:0b</span>
                        <span className="text-blue-600 dark:text-blue-400">eno1</span>
              ...
              {gwInspectorTab === 'remediation' && (
                <div className="space-y-4">
                  <div className="bg-white dark:bg-[#0F172A] p-4 rounded-xl border border-slate-200 dark:border-[#1E293B] space-y-3">
                    <h3 className="font-bold text-slate-900 dark:text-white text-xs">Live Remediation & Administrative Operations</h3>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        disabled={gwActionLoading}
                        className="p-3 bg-gradient-to-br from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-slate-900 dark:text-white rounded-xl text-left transition-all col-span-2 shadow-md"
                      >
                      ...
                      <button
                        ...
                        disabled={gwActionLoading}
                        className="p-3 bg-slate-50 dark:bg-[#090D16] hover:bg-[#162032] border border-slate-200 dark:border-[#1E293B] hover:border-purple-500/50 rounded-xl text-left transition-all"
                      >
                      ...
                      <button
                        ...
                        disabled={gwActionLoading}
                        className="p-3 bg-slate-50 dark:bg-[#090D16] hover:bg-[#162032] border border-slate-200 dark:border-[#1E293B] hover:border-blue-500/50 rounded-xl text-left transition-all"
                      >
                        <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-white">
                      ...
                      <button
                        ...
                        disabled={gwActionLoading}
                        className="p-3 bg-slate-50 dark:bg-[#090D16] hover:bg-[#162032] border border-slate-200 dark:border-[#1E293B] hover:border-emerald-500/50 rounded-xl text-left transition-all"
                      >
                        <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-white">
                      ...
                      <button
                        ...
                        disabled={gwActionLoading}
                        className="p-3 bg-slate-50 dark:bg-[#090D16] hover:bg-[#162032] border border-slate-200 dark:border-[#1E293B] hover:border-amber-500/50 rounded-xl text-left transition-all"
                      >
                        <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-white">
                    ...
                    {gwActionResult && gwActionResult.action !== 'PING' && (
                      <div className="p-3.5 bg-[#05080E] rounded-xl border border-slate-200 dark:border-[#1E293B] font-mono text-[11px] text-slate-600 dark:text-slate-300 space-y-1 mt-3">
                        <div className="flex justify-between text-slate-500">
                          <span>EXECUTION LOG: {gwActionResult.action}</span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">{gwActionResult.status}</span>
                        </div>
            ...
            {/* Footer */}
            <div className="bg-white dark:bg-[#0F172A] px-6 py-3 border-t border-slate-200 dark:border-[#1E293B] flex items-center justify-between">
