  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">NOC Operations & Telemetry Overview</h1>
          <p className="text-xs text-slate-400 mt-0.5">Real-time status of multi-tenant enterprise control-plane fabric</p>
        </div>
        <div className="flex items-center gap-3">
          {liveEvent && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-950/60 border border-amber-800 text-amber-300 text-xs animate-bounce">
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>LIVE: {liveEvent.data.title || 'New Network Event'}</span>
            </div>
          )}
          <Link
            href="/gateways"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs font-semibold hover:bg-emerald-500/20 transition-all"
          >
            <Radar className="w-3.5 h-3.5 animate-spin" style={{ animationDuration: '4s' }} />
            <span>Real Network Fleet</span>
          </Link>
          <Link
            href="/network-map"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-xs font-semibold hover:bg-cyan-500/20 transition-all"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        />
        <MetricCard title="Gateways" value={summary?.gateways ?? 0} href="/gateways" icon={<Cpu className="w-4 h-4" />} badge="Enrolled" />
        <MetricCard title="PoPs" value={summary?.pops ?? 0} href="/pops" icon={<Network className="w-4 h-4" />} badge="Core PoPs" />
        <MetricCard title="Aggregators" value={summary?.aggregators ?? 0} href="/aggregators" icon={<Server className="w-4 h-4" />} badge="Hubs" />
        <MetricCard title="Tunnels" value={summary?.activeTunnels ?? 0} href="/tunnels" icon={<Shuffle className="w-4 h-4" />} badge="Encrypted" />
      </div>

      {/* Second Row KPIs */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Site Health */}
        <div className="bg-[#121824] border border-[#222E45] rounded-lg p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-400 uppercase">Site Connectivity Health</span>
          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-slate-400">Degraded WAN/Tunnels</span>
        ...
        <div className="bg-[#121824] border border-[#222E45] rounded-lg p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-400 uppercase">Host Kernel Telemetry</span>
            <span className="text-xs font-mono text-emerald-400 font-semibold">eno1 (1000M)</span>
          </div>
          <p className="text-xs text-slate-400 mb-3">Real OS socket counters & Linux sysfs metrics polling every 5s.</p>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="p-2 bg-[#0B0F17] rounded border border-[#222E45]">
              <span className="block text-slate-500 text-[10px]">CPU Cores</span>
              <span className="font-mono text-white font-bold">{hostTelemetry?.cpuCount ?? 8} Cores</span>
            </div>
            <div className="p-2 bg-[#0B0F17] rounded border border-[#222E45]">
              <span className="block text-slate-500 text-[10px]">RAM Usage</span>
              <span className="font-mono text-cyan-400 font-bold">{hostTelemetry?.memoryUsagePercent ?? 75}%</span>
            </div>
            <div className="p-2 bg-[#0B0F17] rounded border border-[#222E45]">
              <span className="block text-slate-500 text-[10px]">Gateway RTT</span>
              <span className="font-mono text-emerald-400 font-bold">0.15 ms</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#121824] border border-[#222E45] rounded-lg p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-400 uppercase">AI Operations Engine</span>
            <span className="text-xs font-mono text-cyan-400 font-semibold flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-cyan-400" />
              ONLINE
            </span>
          </div>
          <p className="text-xs text-slate-400 mb-3">Continuous anomaly detection and automated RCA across real MySQL time-series.</p>
          <Link
            href="/ai-assistant"
            className="inline-block w-full text-center py-2 px-3 rounded bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 text-xs font-medium border border-cyan-500/30 transition-all"
          >
            Open AI NOC Assistant →
          </Link>
        </div>
      </div>
