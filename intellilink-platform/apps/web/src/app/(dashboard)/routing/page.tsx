...
export default function RoutingPage() {
  ...
  const { data: routingResponse, isLoading, refetch } = useQuery({
    queryKey: ['routing-list', search, page, pageSize],
    queryFn: async () => {
      const res = await apiClient.get(`/routing?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
      return res.data;
    },
    refetchInterval: 5000,
  });

  const data = Array.isArray(routingResponse?.data)
    ? routingResponse.data
    : Array.isArray(routingResponse)
    ? routingResponse
    : [];
  const totalRoutes = routingResponse?.total ?? data.length;
  const totalPages = routingResponse?.totalPages ?? Math.max(1, Math.ceil(totalRoutes / pageSize));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Routing Fabric & BGP Peering</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Total FIB Routes</div>
          <div className="text-lg font-bold text-slate-900 dark:text-white mt-1">{totalRoutes}</div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Active & Converged</div>
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">
            {(data || []).filter((r: any) => r.status === 'ACTIVE').length} Active
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Dynamic BGP Sessions</div>
          <div className="text-lg font-bold text-cyan-600 dark:text-cyan-400 mt-1">
            {(data || []).filter((r: any) => r.protocol === 'BGP').length} Advertised
          </div>
        </div>
        <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] p-3.5 rounded-lg">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Convergence Time</div>
          <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">&lt; 50 ms</div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#121824] border border-slate-200 dark:border-[#222E45] rounded-lg p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">
          FIB State: <span className="text-emerald-600 dark:text-emerald-400 font-bold">KERNEL_SYNC_NOMINAL</span>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:bg-[#161F30] transition-colors">
                  <td className="p-3.5">
                    <div className="font-semibold text-slate-900 dark:text-white font-mono">{item.prefix || '0.0.0.0/0'}</div>
                    <div className="text-[10px] text-slate-500 font-mono mt-0.5">{item.id}</div>
                  </td>
                  <td className="p-3.5 font-mono text-cyan-600 dark:text-cyan-400">{item.nextHop || '10.250.1.1'}</td>
                  ...
                  <td className="p-3.5 font-mono text-slate-500 dark:text-slate-400">{item.metric || 100}</td>
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#1E293B] rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl space-y-0">
            ...
            <div className="bg-white dark:bg-[#0F172A] px-6 py-4 border-b border-slate-200 dark:border-[#1E293B] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Linux Kernel FIB Path & Hop Tracer</h2>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-800/60 font-medium">
              ...
              <button
                ...
                className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1.5 rounded-lg hover:bg-[#1E293B] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4 font-mono text-xs">
              {traceModal.loading ? (
                <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 font-semibold font-sans">Evaluating Linux kernel forwarding table (FIB)...</p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    ...
                    <div className="bg-white dark:bg-[#0F172A] p-3 rounded-xl border border-slate-200 dark:border-[#1E293B]">
                      ...
                      <span className="text-emerald-600 dark:text-emerald-400 font-bold mt-0.5 block">CONVERGED</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-sans">
                      ...
                      <span className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">{traceModal.result?.pathStatus}</span>
                    </div>
                    <pre className="p-3.5 bg-[#05080E] rounded-xl border border-slate-200 dark:border-[#1E293B] text-emerald-600 dark:text-emerald-400 text-[11px] overflow-x-auto leading-relaxed">
          ...
          <div className="bg-slate-50 dark:bg-[#0B0F17] border border-slate-200 dark:border-[#222E45] rounded-xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="bg-white dark:bg-[#121824] px-5 py-4 border-b border-slate-200 dark:border-[#222E45] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Commit Routing Table Entry</h2>
              </div>
            ...
            >
              <div>
                <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Destination Prefix CIDR *</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Next-Hop IP Address *</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Outbound Interface *</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Routing Protocol *</label>
                  <select
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-300 font-medium block mb-1">Route Metric / Priority</label>
                  <input
                    type="number"
